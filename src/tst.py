...
img_src = 'C:\\Users\\Alex\\source\\bgrec\\resources\\origin.jpg'
...
def mouse_event2(event, x, y, flags, param):
    ...
    # 鼠标移动，画图
        ...
        if drawing:
            if param.lb_down:
                cv2.line(param.img_show, last_point,
                         (x, y), (0, 0, 255), 2, -1)
...
if __name__ == '__main__':
    ...
    while (True):
        ...
        if g_img.lb_up or g_img.rb_up:
            ...
            cv2.grabCut(g_img.img_gc, mask, rect, bgdModel,
                        fgdModel, 5, cv2.GC_INIT_WITH_MASK)
            elapsed = (time.process_time() - start)
            mask2 = np.where((mask == 2) | (mask == 0), 0,
                             1).astype('uint8')  # 0和2做背景
            g_img.img_gc = g_img.img_gc * \
                mask2[:, :, np.newaxis]  # 使用蒙板来获取前景区域
